(async function () {
  ...
  // Puxa produção (pontos) da tela prod (3ª coluna)
  ...
  prodRows.forEach(row => {
    ...
    if (tds.length >= 3) {
      ...
      const pointsText = tds[2].textContent.trim().replace(/\./g, "");
      coordToPoints[coord] = parseInt(pointsText, 10);
    }
  });
  ...
  select.addEventListener("change", async function () {
    ...
    // Função para criar barra de progresso simples
    function createProgressBar(value, max) {
      const percent = Math.min((value / max) * 100, 100);
      return `
        <div style="background:#ddd; width: 90px; height: 16px; border-radius: 6px; overflow: hidden;">
    ...
    let output = `<table class="vis" width="100%">
      <thead><tr><th>Nome</th><th style="width:90px;">Coord</th><th style="width:120px;">Produção</th><th>Ações</th></tr></thead><tbody>`;
    let total = 0;
    ...
    rows.forEach(row => {
      ...
      if (tds.length >= 2) {
        ...
        output += `<tr>
          ...
          <td>${createProgressBar(points, MAX_POINTS)}<div style="text-align:center; font-size: 12px; margin-top: -20px; position: relative; font-weight:bold;">${points.toLocaleString()}</div></td>
