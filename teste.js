(async function () {
    ...
    select.addEventListener("change", async function () {
        ...
        // Atualiza o seletor de grupo do jogo para sincronizar a seleção
        const jogoGroupSelect = document.querySelector("#group_select"); // Ajuste se necessário
        if (jogoGroupSelect) {
            jogoGroupSelect.value = groupId;
            // Dispara o evento change para atualizar o grupo no jogo
            jogoGroupSelect.dispatchEvent(new Event("change", { bubbles: true }));
        }
        ...
        let output = `<table class="vis" width="100%">
            <thead><tr><th>Nome</th><th style="width: 100px;">Coordenadas</th><th>Ações</th></tr></thead><tbody>`;
        let total = 0;
        ...
        rows.forEach(row => {
            ...
            if (tds.length >= 2) {
                ...
                output += `<tr>
                    <td>${link}</td>
                    <td style="width: 100px;"><span class="coord-val">${coords}</span></td>
    ...
    // Se houver grupo salvo, já carrega e sincroniza com o jogo
    if (savedGroupId) {
