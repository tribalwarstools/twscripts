// ==UserScript==
// @name         Agendador Avançado (Painel Organizado + Status Dinâmico + Auto-Confirmar)
// @namespace    http://tampermonkey.net/
// @version      3.4
// @description  Agenda múltiplos ataques com contagem regressiva, envio automático, BBCode, tooltip de tropas, painel flutuante reorganizado.
// @author       GiovaniG
...
<div style="margin-bottom:4px;">
  ...
  function renderTable() {
    ...
    tbody.innerHTML = list.map((a,i) => {
      ...
      const status = a.done ? '✅ Enviado' : (parseDateTimeToMs(a.datetime) - now > 0 ? `🕒 ${Math.ceil((parseDateTimeToMs(a.datetime)-now)/1000)}s` : '🔥 Enviando...');
      return `<tr>
  ...
  // === Função de execução de ataque já existente (fetch + auto confirm) ===
...
// ---------- fim executeAttack ----------
  ...
  el('tws-add').onclick = () => {
    ...
    if(!selVal || !alvo || isNaN(parseDateTimeToMs(dt))) return alert('Verifique origem, coordenadas e data!');
  ...
  el('tws-clear').onclick = () => { if(confirm('Apagar todos os agendamentos?')){ localStorage.removeItem(STORAGE_KEY); renderTable(); status.textContent='Lista limpa.'; } };
